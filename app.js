...
app.get("/getEarthquake", function(req, res) {
  fs.readFile("./Source/earthquakes_month_cleaned.geojson", function(
    err,
    data
  ) {
    if (err) {
...
let convertEarthquakeData = function() {
  return exec(
    "geojson-pick mag time place < Source/earthquakes_month.geojson > Source/earthquakes_month_cleaned.geojson",
    err => {
      if (err) {
...
earthquakesData();
setInterval(earthquakesData, 300000);
