mapboxgl.accessToken =
  "pk.eyJ1IjoibmloZTA3NjAiLCJhIjoiY2p1bzhmOWI1MnJsODQxb2FnaTdmb3J0NyJ9.pPdj9B-Ql2FTGrqZsbceEg";
...
async function drawEarthquakeNumbers() {
  ...
  let node = document.createTextNode(
    "Number of earthquakes within the last 30 days: " +
      (await $.get(earthquakeNumber)) +
      "."
  );
  document
...
map.on("load", function() {
  ...
  map.on("mouseenter", "earthquakes-point", function(e) {
    ...
    popup = new mapboxgl.Popup({ offset: [0, 0] })
      ...
      .setHTML(
        "<h3>Earthquake Detail</h3>" +
          "<p><b>Magnitude: </b>" +
          feature.properties.mag.toFixed(2) +
          "</p>" +
          "<p><b>Time: </b>" +
  ...
  document.getElementById("slider").addEventListener("input", function(e) {
    ...
    if (mag === 1) {
      filterMagnitude = [
        "all",
        [">=", ["number", ["get", "mag"]], mag - 0.9],
        ["<=", ["number", ["get", "mag"]], mag + 0.5]
      ];
    } else if (mag === 2) {
      filterMagnitude = [
        "all",
